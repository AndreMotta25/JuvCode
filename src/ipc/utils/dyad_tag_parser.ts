...
export function getDyadRunScriptTags(
  fullResponse: string,
): { script: string; args?: string }[] {
  const results: { script: string; args?: string }[] = [];
  const tagRegex = /<dyad-command[^>]*type=(?:"run-script"|'run-script')[^>]*>([\s\S]*?)<\/dyad-command>/g;
  let match;
  while ((match = tagRegex.exec(fullResponse)) !== null) {
    ...
    const scriptAttr = tagText.match(/script=(?:"([^"]+)"|'([^']+)')/);
    if (scriptAttr) script = scriptAttr[1] || scriptAttr[2];
    const argsAttr = tagText.match(/args=(?:"([^"]+)"|'([^']+)')/);
    if (argsAttr) args = argsAttr[1] || argsAttr[2];
    if (!script && inner) script = inner;
    if (script) results.push({ script, args });
  }
  return results;
}
