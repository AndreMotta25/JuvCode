  ...
  return withLock("get-proposal:" + chatId, async () => {
    ...
    try {
      ...
      if (latestAssistantMessage?.content) {
        ...
        const runScripts = getDyadRunScriptTags(latestAssistantMessage.content);
        for (const rs of runScripts) {
          actions.push({ id: "run-script", script: rs.script, args: rs.args });
        }
      }
