...
} from "../../supabase_admin/supabase_management_client_fixed";
...
export function registerAppHandlers() {
  ...
  handle(
    ...
    async (
      _,
      params: { appId: number; installCommand: string; startCommand: string },
    ): Promise<void> => {
      ...
      if (!installCommand?.trim() || !startCommand?.trim()) {
        throw new Error(
          "Both installCommand and startCommand are required to override defaults",
        );
      }
      await db
        .update(apps)
        .set({ installCommand, startCommand })
  ...
  ipcMain.handle(
    ...
    async (
      event: Electron.IpcMainInvokeEvent,
      { appId, script, args }: { appId: number; script: string; args?: string },
    ): Promise<void> => {
      return withLock(appId, async () => {
        ...
        if (!fs.existsSync(pkgPath)) throw new Error("package.json not found in app");
        let pkg: any;
        try {
        ...
        if (!scripts[script]) {
          throw new Error(`Script not found in package.json: ${script}`);
        }
        const cmd = `npm run ${script}${args?.trim() ? " " + args.trim() : ""}`;
        const spawnedProcess = spawn(cmd, [], {
          cwd: appPath,
          shell: true,
          stdio: "pipe",
          detached: false,
        ...
        if (!spawnedProcess.pid) {
          let errorOutput = "";
          spawnedProcess.stderr?.on("data", (d) => (errorOutput += d.toString()));
          await new Promise<void>((resolve) => {
            spawnedProcess.once("error", () => resolve());
          });
          throw new Error(`Failed to spawn script: ${errorOutput}`);
        }
        ...
        spawnedProcess.stderr?.on("data", (data) => {
          const message = (data || "").toString();
          safeSend(event.sender, "app:output", { type: "stderr", message, appId });
        });
        spawnedProcess.on("close", (code, signal) => {
          const message = `Script '${script}' finished with code ${code}, signal ${signal}`;
          safeSend(event.sender, "app:output", { type: "stdout", message, appId });
        });
      });
    },
