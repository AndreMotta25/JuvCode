...
`;
        ...
        const limitedHistoryChatMessages = limitedMessageHistory.map((msg) => ({
          ...
          content:
            settings.selectedChatMode === "ask" || settings.selectedChatMode === "arquiteto"
        ...
        const { fullStream } = await simpleStreamText({
          ...
          modelClient,
          files: files,
          dyadDisableFiles: settings.selectedChatMode === "arquiteto",
        });
        ...
        try {
          ...
          if (settings.selectedChatMode === "arquiteto") {
            const hasWriteTags =
              getDyadWriteTags(fullResponse).length > 0 ||
              getDyadRenameTags(fullResponse).length > 0 ||
              getDyadDeleteTags(fullResponse).length > 0 ||
              getDyadAddDependencyTags(fullResponse).length > 0;
            const shouldSwitchToBuild =
              fullResponse.includes("[[SWITCH_TO_BUILD]]") || hasWriteTags;

            // Sanitize architect response: remove any dyad tags so build does not assume files exist
            const architectContent = removeDyadTags(fullResponse).replace(
              /\[\[SWITCH_TO_BUILD\]\]/g,
              "",
            );
            fullResponse = architectContent;
            await db
              .update(messages)
              .set({ content: fullResponse })
              .where(eq(messages.id, placeholderAssistantMessage.id));
            ...
            if (shouldSwitchToBuild) {
              try {
                writeSettings({ selectedChatMode: "build" });
                settings.selectedChatMode = "build" as any;
                safeSend(event.sender, "settings:updated", readSettings());

                const { fullStream: buildFollowStream } = await simpleStreamText({
                  chatMessages: [
                    ...chatMessages,
                    { role: "assistant", content: fullResponse },
                    {
                      role: "user",
                      content:
                        "Ignore any previous claims of created files. They do not exist yet. Implement the architecture plan now, generating the necessary files and components.",
                    },
                  ],
                  modelClient,
                  files: files,
                ...
                const followResult = await processStreamChunks({
                  fullStream: buildFollowStream,
                  fullResponse,
                  abortController,
                  chatId: req.chatId,
                  processResponseChunkUpdate,
                });
                fullResponse = followResult.fullResponse;
              } catch (e) {
                logger.warn(
                  "Failed to continue in build after architect trigger:",
                  e,
                );
              }
            }
          ...
          if (
            settings.selectedChatMode === "arquiteto" &&
            fullResponse.includes("[[SWITCH_TO_BUILD]]")
          ) {
            try {
              fullResponse = fullResponse.replace(/\[\[SWITCH_TO_BUILD\]\]/g, "");
              await db
                .update(messages)
                .set({ content: fullResponse })
                .where(eq(messages.id, placeholderAssistantMessage.id));
              ...
              const { fullStream: buildFollowStream } = await simpleStreamText({
                chatMessages: [
                  ...
                  { role: "assistant", content: fullResponse },
                  {
                    role: "user",
                    content:
                      "Proceed to implement the architecture plan above in code. Create the necessary files and components to start execution.",
                  },
                ],
                modelClient,
                files: files,
              ...
              logger.warn("Failed to switch mode to build after architect token:", e);
            }
          }
