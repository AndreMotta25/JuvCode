...
export class IpcClient {
  ...
  private constructor() {
    ...
    this.ipcRenderer.on("settings:updated", (data) => {
      const callbacks = this.settingsUpdatedCallbacks || [];
      try {
        const settings = data as UserSettings;
        callbacks.forEach((cb) => cb(settings));
      } catch (err) {
        console.warn("[IPC] Invalid settings:updated payload", err);
      }
    });
  }
  ...
  public async runScript(params: { appId: number; script: string; args?: string }): Promise<void> {
    if (!params.script || !params.script.trim()) {
      throw new Error("Script inválido ou ausente");
    }
    await this.ipcRenderer.invoke("app:run-script", params);
  }
  ...
  private settingsUpdatedCallbacks?: Array<(settings: UserSettings) => void>;

  public onSettingsUpdated(
    callback: (settings: UserSettings) => void,
  ): () => void {
    if (!this.settingsUpdatedCallbacks) this.settingsUpdatedCallbacks = [];
    this.settingsUpdatedCallbacks.push(callback);
    return () => {
      this.settingsUpdatedCallbacks = (this.settingsUpdatedCallbacks || []).filter(
        (cb) => cb !== callback,
      );
    };
  }
  ...
  public async setAppCommands(params: {
    appId: number;
    installCommand: string;
    startCommand: string;
  }): Promise<void> {
    await this.ipcRenderer.invoke("set-app-commands", params);
  }
  ...
  public async importMcpConfig(json: string): Promise<{ success: boolean; path: string }> {
    return this.ipcRenderer.invoke("mcp:import-json", { json });
  }

  public async exportMcpConfig(): Promise<{ json: any; path: string }> {
    return this.ipcRenderer.invoke("mcp:export-json");
  }
  ...
  // --- Terminal Methods ---
  public async executeTerminalCommand(params: {
    command: string;
    cwd: string;
    sessionId: string;
  }): Promise<{
    ...
    const result = await this.ipcRenderer.invoke("terminal:execute-command", params);
    
    // Map handler result to expected format
    return {
  ...
  public async cancelTerminalCommand(params: {
    commandId: string;
    sessionId: string;
  }): Promise<{ success: boolean; error?: string }> {
    return this.ipcRenderer.invoke("terminal:cancel-command", params);
  }

  public async getTerminalCwd(sessionId?: string): Promise<string> {
    return this.ipcRenderer.invoke("terminal:get-cwd", { sessionId });
  }

  public async getTerminalEnv(): Promise<Record<string, any>> {
    return this.ipcRenderer.invoke("terminal:get-env");
  }
  public async clearTerminal(sessionId: string): Promise<{ success: boolean; error?: string }> {
    return this.ipcRenderer.invoke("terminal:clear", { sessionId });
  }
}
